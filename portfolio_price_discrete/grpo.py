...
def grpo_update(agent, rollout_groups, env, optimizer):
    ...
    for group in rollout_groups:
        ...
        std_r = group_returns.std(unbiased=True) + 1e-8  # 수치 안정성 보장
    ...
    for global_step in range(total_steps):
        ...
        for grpo_iter in range(env.mu):
            ...
            with torch.no_grad():
                ...
                state = state.reshape(1, -1)
                state = torch.tensor(state, dtype=torch.float32)
                strategy_returns, strategy_weights = eval_partial_backtests(df_long=valid_env.real_data,
...
def test_grpo(env, model):
    ...
    state = state.reshape(1, -1)
    state = torch.tensor(state, dtype=torch.float32)
    strategy_returns, strategy_weights = eval_partial_backtests(df_long=env.real_data,
