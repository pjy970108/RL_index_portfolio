...
import numpy as np
from agent import DiscreteSACAgent, EarlyStopping
import copy
from tqdm import tqdm
import wandb
import time 
from backtesting import eval_partial_backtests_v2  # 함수만 명시적으로 import
import eval_metric
    ...
    for epoch in tqdm(range(env.epochs + 1)):
        ...
        for episode in range(env.batch_samples):
            ...
            while not done:
                ...
                total_sub_reward.append(reward)
                update_counter += 1
                if len(agent.replay_buffer) > env.mini_batch_size and update_counter % env.update_interval == 0:
                    for _ in range(env.mu):
        ...
        with torch.no_grad():
            ...
            state = state.reshape(1, -1)
            state = torch.tensor(state, dtype=torch.float32).to(env.device)
            strategy_returns, strategy_weights = eval_partial_backtests_v2(
                index_long=valid_env.index_df,
            ...
            for _ in range(valid_env.max_step):
                ...
                next_state, combined_returns, done, _, _ = valid_env.eval_step(action, strategy_returns)
                next_state = next_state.reshape(1, -1)
                state = next_state.clone()
            ...
            valid_sharpe = eval_metric.calculate_sharpe_ratio(
                model_return, risk_free_rate=valid_env.risk_free_rate, annual_factor=valid_env.annual_factor)

        avg_main_episode_reward = np.mean(main_episode_rewards)

        wandb.log({
            "train_episode_reward": avg_main_episode_reward,
            "valid_sharpe": valid_sharpe
        })

        if (epoch + 1) % 2 == 0:
            agent.save(env.save_path)
