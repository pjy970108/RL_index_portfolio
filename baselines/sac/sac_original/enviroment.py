...
from backtesting import run_partial_backtests_v2  # 함수만 명시적으로 import
...
class Stock_Env:
    def __init__(self, config, states, index_df, future_df, eval=False):
        ...
        self.dynamic_dict = {
            "future_risk": dp.backtest_strategy,
            "index_risk": dp.backtest_strategy,
            "future_minvar": dp.backtest_strategy,
            "index_minvar": dp.backtest_strategy,
            "future_ms": dp.backtest_strategy,
            "index_ms": dp.backtest_strategy,
            "future_paa": dp.backtest_paa_from_pivot,
            "index_paa" : dp.backtest_paa_from_pivot
        }
        self.all_df = pd.concat([self.index_df[["ticker", "close"]], self.future_df[["ticker", "close"]]], axis=0)
        ...
        self.lr_actor = float(config["lr_actor"])
        self.lr_critic = float(config["lr_critic"])
        ...
        self.tau = config["tau"]
        self.automatically_tune_entropy_hyperparameter = config.get("automatically_tune_entropy_hyperparameter", False)
        self.entropy_term_weight = config.get("entropy_term_weight", 0.01)
        self.add_extra_noise = config.get("add_extra_noise", False)
        self.do_evaluation_iterations = config.get("do_evaluation_iterations", True)
    ...
    def step(self, actions, group_reset = False):
        ...
        model_eval_metric["combined"] = combined
