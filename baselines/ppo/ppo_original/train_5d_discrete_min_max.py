...
from grpo_min_max import train_with_grpo, optimize_model_memory
...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./portfolio_price_discrete_v2/config/train_config.yaml", "r") as f:
        config = yaml.safe_load(f)
    
    with open("./portfolio_price_discrete_v2/config/test_config.yaml", "r") as f:
    ...
    #4. Test 데이터 로드
    train_for_valid = pd.read_csv(test_config["data_paths"]["train_csv"], index_col=0)
    test_df = pd.read_csv(test_config["data_paths"]["test_csv"], index_col=0)
    test_all_df = pd.concat([train_for_valid, test_df], axis=0)
    ...
    valid_env = Stock_Env(config=test_config, states=valid_tensor, real_df=test_all_df, eval=True)
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./portfolio_price_discrete_v2/model/{traj_len}_minmax/'
    ...
    save_filename = (
        ...
        f'_num_steps{config["num_steps"]}_reward_cond_{config["reward_cond"]}_grpo_model_minmax.pth'
    )
...
if __name__ == "__main__":
    set_seed(42)
    sweep_id = wandb.sweep(sweep_config, project="grpo_portfolio_5d_discrete_minmax_v2")
