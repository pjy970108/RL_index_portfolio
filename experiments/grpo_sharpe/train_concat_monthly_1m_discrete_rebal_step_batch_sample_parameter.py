...
sweep_config = {
    ...
    'parameters': {
        ...
        'lr_actor': {'values': [1e-6, 5e-6]},
        # 'clip_grad': {'values': [0.5, 0.1]},
        'epsilon': {'values': [0.15, 0.1, 0.2]},
        'beta': {'values': [0.0]},
        'mu': {'values': [100]},
...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./portfolio_price_discrete_rebal_step_1m_concat_asset/config/train_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./portfolio_price_discrete_rebal_step_1m_concat_asset/model/{traj_len}_parameter_v1/'
...
if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep_config, project="grpo_portfolio_concat_asset_1m_monthly")
