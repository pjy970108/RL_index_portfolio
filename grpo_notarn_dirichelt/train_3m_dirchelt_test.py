...
from grpo_test import train_with_grpo, optimize_model_memory
from agent_test import PPO, ForwardableActorCritic
import copy
...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./grpo_notarn_dirichelt/config/train_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./grpo_notarn_dirichelt/model/dirichlet_{traj_len}/'
    os.makedirs(model_path, exist_ok=True)

    save_filename = (
        f'{traj_len}_actor_lr{config["lr_actor"]}'
        f'_clip_grad{config["clip_grad"]}_epslion_{config["epslion"]}'
        f'beta{config["beta"]}_mu{config["mu"]}_num_group{config["num_trajectories"]}'
    ...
    env = Stock_Env(config=config, states=train_tensor, real_df=train_df)
    # for i in range(0, env.states.shape[0], 20):
    #     diff = (env.states[i] - env.states[i+20]).abs().mean().item()
    #     print(f"State diff [{i}-{i+20}]: {diff:.4f}")
    ...
    samples = []
    for i in range(0, 64):
        s = env.states[i].unsqueeze(0).to(device)
        s = s.reshape(1, -1)
        raw_alpha, _ = agent.policy.act(s)
        alpha = F.softplus(raw_alpha) * agent.alpha_param
        dist = Dirichlet(alpha)
        action = dist.sample()
        samples.append(action)
        expected = alpha / alpha.sum(dim=-1, keepdim=True)
        # print(f"[{i}] expected_action: {expected.tolist()}")
...
if __name__ == "__main__":
    set_seed(42)
    sweep_id = wandb.sweep(sweep_config, project="grppo_index_3m_notarn_dirichlet_test")
