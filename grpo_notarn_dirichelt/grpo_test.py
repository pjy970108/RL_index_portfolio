...
from agent_test import PPO
from enviroment import Stock_Env
import wandb
from torch.utils.data import Dataset, DataLoader
from joblib import Parallel, delayed
import sys
from backtesting import eval_partial_backtests  # 함수만 명시적으로 import
import pandas as pd
    ...
    for _ in range(env.num_trajectories):
        ...
        with torch.no_grad():
            for _ in range(env.update_interval):
                ...
                _, ref_logprob = ref_model.policy_old.act(state)
...
def generate_rollout_data(model, ref_model, env, iteration):
    ...
    if iteration == 0:
        ...
        ref_state = model.policy_old.state_dict()
    else:
        model_state = model.policy_old.state_dict()
...
def grpo_update(agent, rollout_groups, env, optimizer):
    ...
    for group in rollout_groups:
        ...
        std_r = group_returns.std(unbiased=False) + 1e-8  # 수치 안정성 보장

        for traj in group:
            traj_reward_norm = (traj["rewards"]-mean_r) / std_r
            traj["advantage"] = traj_reward_norm
    ...
    for batch in loader:
        ...
        log_probs = ref_log_probs.squeeze(-1)
...
def train_with_grpo(agent, env):
    ...
    for iteration in range(env.num_iterations):
        # if hasattr(env, 'beta_schedule') and iteration < len(env.beta_schedule):
        #     env.beta = env.beta_schedule[iteration]
...
def test_grpo(env, model):
    ...
    state = state.reshape(1, -1)
    state = torch.tensor(state, dtype=torch.float32)
    strategy_returns = eval_partial_backtests(df_long=env.real_data,
            trading_data=env.days[0],
            top_n=env.top_n,
            look_backs=env.look_back,
            rebalance_every=env.window_size,
            cost=env.cost,
            top_pct = env.top_pct,
            risk_coefficient=env.risk_coefficient)  # 평가 함수 호출
    ...
    return model_return, strategy_returns, weight_model
