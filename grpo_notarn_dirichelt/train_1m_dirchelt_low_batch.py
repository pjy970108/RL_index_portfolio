...
def main_wandb():
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./grpo_notarn_dirichelt/model/dirichlet_low_batch_{traj_len}/'
    os.makedirs(model_path, exist_ok=True)

    save_filename = (
        f'{traj_len}_actor_lr{config["lr_actor"]}'
        f'_clip_grad{config["clip_grad"]}_epslion_{config["epslion"]}'
        f'batch_samples{config["batch_samples"]}_mu{config["mu"]}_num_group{config["num_trajectories"]}'
        f'_num_steps{config["num_steps"]}_reward_cond_{config["reward_cond"]}_TARN_grpo_model.pth'
    )
...
if __name__ == "__main__":
    set_seed(42)
    sweep_id = wandb.sweep(sweep_config, project="grppo_index_1m_notarn_dirichlet_v3")
