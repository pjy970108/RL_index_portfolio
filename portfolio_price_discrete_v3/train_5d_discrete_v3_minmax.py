...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./portfolio_price_discrete_v3/config/train_config.yaml", "r") as f:
        config = yaml.safe_load(f)
    
    with open("./portfolio_price_discrete_v3/config/test_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./portfolio_price_discrete_v3/model/{traj_len}/'
...
if __name__ == "__main__":
    set_seed(42)
    sweep_id = wandb.sweep(sweep_config, project="grpo_portfolio_5d_discrete_v3_minmax")
