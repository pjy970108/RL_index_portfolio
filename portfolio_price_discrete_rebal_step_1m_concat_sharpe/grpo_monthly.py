        ...
        for grpo_iter in range(env.mu):
            ...
            with torch.no_grad():
                ...
                for _ in range(valid_env.max_step):
                    ...
                    next_state, combined_returns, done, action_dict = valid_env.eval_step(action.cpu().numpy(), strategy_returns)
                    next_state = next_state.reshape(1, -1)
                    state = next_state.clone()
...
def test_grpo(env, model):
    ...
    for _ in range(env.max_step):
        ...
        next_state, combined_returns, done, action_dict = env.eval_step(action.cpu().numpy(), strategy_returns)
