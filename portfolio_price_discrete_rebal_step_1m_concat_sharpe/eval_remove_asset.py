...
if __name__ == "__main__":
    ...
    train_data_path = "data/train_del_asset_v1.csv"
    test_data_path = "data/test_del_asset_v1.csv"
    test_pt = "data/portfolio_price/concat_portfolio_test_monthly_v2_remove.pt"
    
    device = torch.device(config.get("device", "cuda:0"))
    config["device"] = device
    
    index_train_for_valid = pd.read_csv(config["data_paths"]["index_train_csv"])
    index_test_df = pd.read_csv(config["data_paths"]["index_test_csv"])
    ...
    future_train_for_valid = pd.read_csv(config["data_paths"]["future_train_csv"])
    future_test_df = pd.read_csv(config["data_paths"]["future_test_csv"])
