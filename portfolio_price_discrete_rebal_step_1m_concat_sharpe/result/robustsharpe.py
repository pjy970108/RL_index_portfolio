...
def block_size_calibrate(returns: npt.NDArray[np.float64], b_vec: List = [1, 3, 6, 10], alpha: float = 0.05,
    ...
    for m, _resid_mat_star in enumerate(bs.bootstrap(K)):
        ...
        for t in range(1, T + T_start - 1):
            var_data[t, 0] = coef1[0] + coef1[1] * var_data[t - 1, 0] + coef1[2] * var_data[t - 1, 1] + resid_mat_star[
                t, 0]
            var_data[t, 1] = coef2[0] + coef2[1] * var_data[t - 1, 0] + coef2[2] * var_data[t - 1, 1] + resid_mat_star[
                t, 1]
    ...
    emp_reject_prob /= K
    # α와의 차이가 가장 작은 b 선택
    deltas = np.abs(emp_reject_prob - alpha)
    b_opt = int(b_vec[int(np.argmin(deltas))])
    return b_opt, emp_reject_prob, deltas
...
def bootstrap_inference(returns: npt.NDArray[np.float64], block_size: int, alpha: float = 0.05, M: int = 100,
    ...
    for m, ret_star_boot in enumerate(bs.bootstrap(M)):
        ...
        y_star = np.column_stack([
            ret_star[:, 0] - mu_hat_star[0],
            ret_star[:, 1] - mu_hat_star[1],
            ret_star_squared[:, 0] - sigma_sq_hat_star[0],
            ret_star_squared[:, 1] - sigma_sq_hat_star[1],
        ])

        psi_hat_star = np.zeros((4, 4), dtype=float)
        for j in range(1, l+1):  # ← 마지막 블록 포함
            sl = slice((j - 1) * block_size, j * block_size)
            zeta_star = (block_size ** 0.5) * y_star[sl, :].mean(axis=0)
            psi_hat_star += np.outer(zeta_star, zeta_star)  # ← .T 불필요
    ...
    if return_resid:
        return SR_hat, SR_diff, (SR_diff - ci, SR_diff + ci), p_value, se, d_star_arr_non_abs, d
    else:
        return SR_hat, SR_diff, (SR_diff - ci, SR_diff + ci), p_value, se, d
