...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./grpo_notarn_softmax/config/train_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./grpo_notarn_softmax/model/{traj_len}_minmax/'
    os.makedirs(model_path, exist_ok=True)

    save_filename = (
        f'{traj_len}_actor_lr{config["lr_actor"]}'
        f'_clip_grad{config["clip_grad"]}_epslion_{config["epslion"]}'
        f'batch_samples_{config["batch_samples"]}_mu{config["mu"]}_num_group{config["num_trajectories"]}'
...
if __name__ == "__main__":
    set_seed(42)
    sweep_id = wandb.sweep(sweep_config, project="grppo_index_5d_notarn_softmax")
