...
class PPO:
    def __init__(self, config):
        ...
        self.policy = ActorCritic(self.asset_dim, self.action_dim, self.has_continuous_action_space, self.action_std_init, self.hyperparameters, self.device, self.alpha_param).to(self.device)
        ...
        self.policy_old = ActorCritic(self.asset_dim, self.action_dim, self.has_continuous_action_space, self.action_std_init, self.hyperparameters, self.device, self.alpha_param).to(self.device)
    ...
    def select_action(self, state, deterministic=False):
        ...
        if self.has_continuous_action_space:
            with torch.no_grad():
                ...
                action, action_logprob, attn_scores = self.policy_old.act(state, deterministic)
        ...
        return action.detach().cpu().numpy().flatten(), action_logprob, attn_scores
    ...
    def load(self):
        ...
        raw_state = torch.load(self.save_path, map_location=self.device)
