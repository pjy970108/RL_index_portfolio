        ...
        for gen_idx in range(env.num_trajectories):
            ...
            with torch.no_grad():
                ...
                for day_step in range(env.update_interval):
                    ...
                    trajectory["actions"].append(torch.tensor(actions, dtype=torch.float32))
                    trajectory["log_probs"].append(action_logprob.detach())
                    trajectory["rewards"].append(rewards)
                    trajectory["ref_log_probs"].append(ref_log_probs.detach())
...
def grpo_update(agent, rollout_groups, env, optimizer):
    ...
    for batch in loader:
        ...
        all_rewards.append(batch["rewards"].mean(dim=1).sum())
