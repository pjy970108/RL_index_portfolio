...
class ActorCritic(nn.Module):
    ...
    def __init__(self, feature, action_dim, has_continuous_action_space, action_std_init, hyperparameters, device, alpha_param):
    ...
    def act(self, state, deterministic=False):
        ...
        if self.has_continuous_action_space:
            ...
            return action.detach(), action_logprob.detach(), attn_scores
        # if self.has_continuous_action_space:
        #     # raw_alpha = self.actor(state)
        #     # alpha = F.softplus(raw_alpha) * self.alpha_param + 1e-3  # 극단적 분포 유도
        #     # dist = Dirichlet(alpha)
        #     # action_probs = self.actor(state)  # 이미 softmax 포함됨

        #     if deterministic:
        #         # action = alpha / alpha.sum(dim=-1, keepdim=True)  # Expected value
        #         return action_probs.detach(), None, attn_scores  # 확률(weight) 자체를 리턴
        ...
        return action.detach().squeeze(), action_logprob.detach(), attn_scores
