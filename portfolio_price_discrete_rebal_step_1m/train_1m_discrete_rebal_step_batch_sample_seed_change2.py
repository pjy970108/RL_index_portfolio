...
sweep_config = {
    ...
    'parameters': {
        ...
        "seed" : {'values': [36, 37, 38, 39, 43, 45, 46, 48, 49, 50, 51, 52, 53, 54, 56, 57, 58]}
    }
}
...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./portfolio_price_discrete_rebal_step_1m/config/train_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./portfolio_price_discrete_rebal_step_1m/model/{traj_len}_seed/'
...
if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep_config, project="grpo_portfolio_rebal_minmax_1m")
