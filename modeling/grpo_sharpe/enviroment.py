...
class Stock_Env:
    def __init__(self, config, states, index_df, future_df, eval=False):
        # 상속받은 EnvConfig 초기화
        # self.current_idx = 0
        ...
        self.dynamic_dict = {
            "risk_parity": dp.backtest_strategy,
            "min_var": dp.backtest_strategy,
            "max_sharpe": dp.backtest_strategy,
            "paa": dp.backtest_paa_from_pivot
        }
        self.all_df = pd.concat([self.index_df[["ticker", "close", "paa_score"]], self.future_df[["ticker", "close", "paa_score"]]], axis=0)
        self.all_df.sort_index(inplace=True)
    ...
    def reset(self):
        ...
        if self.eval:
            ...
            # self.idx =  len(self.days) - self.update_interval
            
            self.idx = random.randint(0, len(self.days) - self.update_interval)
            # self.idx = 1502
        # self.idx = 51
        # self.idx = 1429
        # 투자금/weights 초기화
    ...
    def simulate_combined_portfolio_returns(self, top3_action: dict ) -> pd.Series:
        ...
        equal_returns = None
        # print("top3_action : ", top3_action)
        # print("self.strategy_returns : ", self.strategy_returns)
    ...
    #     try:
    #         combined_returns, equal_returns = self.simulate_combined_portfolio_returns(top3_action)

    #         if not isinstance(combined_returns, pd.Series):
    #             print("[WARNING] combined_returns is invalid or empty. top3_action:", top3_action)
    #             print("days : ",  list(self.days)[self.idx])

    #         if not isinstance(equal_returns, pd.Series):

    #             print("[WARNING] equal_returns is invalid or empty. top3_action:", top3_action)
    #             print("days : ",  list(self.days)[self.idx])
    ...
    #         print("[ERROR] Exception occurred in calculate_action_return")
    #         print("top3_action:", top3_action)
    #         print("combined_returns:", combined_returns if 'combined_returns' in locals() else 'undefined')
    #         print("equal_returns:", equal_returns if 'equal_returns' in locals() else 'undefined')
    #         print("Error:", str(e))
    #         raise  # re-raise to propagate error if needed
    ...
    def step(self, actions, group_reset = False):
        ...
        self.strategy_returns = run_partial_backtests_v2(
            index_long=self.index_df,
            future_long = self.future_df,
            equal_long =  self.all_df,
            current_date=day,
            top_n=self.top_n,
    ...
    def eval_step(self, actions, strategy_returns):
        ...
        top3_action = self.top3_action(actions)
        combined_returns, equal_returns = self.simulate_combined_portfolio_returns(top3_action)    
        cumulative_ret = (1 + combined_returns)
        invest_money = self.rewards[-1]
        invest_result = invest_money * cumulative_ret
        
        cumulative_equal = (1 + equal_returns)
        equal_money = self.n_rewards[-1]
        equal_result = equal_money * cumulative_equal
        ...
        if done:
            ...
            # else:
            #     self.reset()
            return state, combined_returns, done, save_action_dict
