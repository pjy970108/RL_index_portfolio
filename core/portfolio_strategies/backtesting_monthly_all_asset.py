...
def run_partial_backtests_v2(
    ...
    try:
        strategies['future_risk'] = backtest_strategy(future_pivot_return, compute_risk_parity_weight_from_window, rebalance_every, look_backs, cost)
        
        strategies['index_risk'] = backtest_strategy(index_pivot_return, compute_risk_parity_weight_from_window, rebalance_every, look_backs, cost)
        strategies['future_minvar'] = backtest_strategy(future_pivot_return, compute_minvar_weights, rebalance_every, look_backs, cost)
        strategies['index_minvar'] = backtest_strategy(index_pivot_return, compute_minvar_weights, rebalance_every, look_backs, cost)
        strategies['future_ms'] = backtest_strategy(future_pivot_return, compute_max_sharpe_min_var, rebalance_every, look_backs, cost, risk_free_rate=risk_free_rate)
        strategies['index_ms'] = backtest_strategy(index_pivot_return, compute_max_sharpe_min_var, rebalance_every, look_backs, cost, risk_free_rate=risk_free_rate)
        strategies['future_paa'] = backtest_paa_from_pivot(future_pivot_return, future_pivot_paa_score, look_backs, rebalance_every, top_n, 0.0, cost)
        strategies['index_paa'] = backtest_paa_from_pivot(index_pivot_return, index_pivot_paa_score, look_backs, rebalance_every, top_n, 0.0, cost)
...
def eval_partial_backtests_v2(
    ...
    try:
        strategies['future_risk'] = backtest_strategy(future_pivot_return, compute_risk_parity_weight_from_window, rebalance_every, look_backs, cost, rebalance_every)
        
        strategies['index_risk'] = backtest_strategy(index_pivot_return, compute_risk_parity_weight_from_window, rebalance_every, look_backs, cost, rebalance_every)
        strategies['future_minvar'] = backtest_strategy(future_pivot_return, compute_minvar_weights, rebalance_every, look_backs, cost, rebalance_every)
        strategies['index_minvar'] = backtest_strategy(index_pivot_return, compute_minvar_weights, rebalance_every, look_backs, cost, rebalance_every)
        strategies['future_ms'] = backtest_strategy(future_pivot_return, compute_max_sharpe_min_var, rebalance_every, look_backs, cost, rebalance_every, risk_free_rate=risk_free_rate)
        strategies['index_ms'] = backtest_strategy(index_pivot_return, compute_max_sharpe_min_var, rebalance_every, look_backs, cost, rebalance_every, risk_free_rate=risk_free_rate)
        strategies['future_paa'] = backtest_paa_from_pivot(future_pivot_return, future_pivot_paa_score, look_backs, rebalance_every, top_n, 0.0, cost, rebalance_every)
        strategies['index_paa'] = backtest_paa_from_pivot(index_pivot_return, index_pivot_paa_score, look_backs, rebalance_every, top_n, 0.0, cost, rebalance_every)
