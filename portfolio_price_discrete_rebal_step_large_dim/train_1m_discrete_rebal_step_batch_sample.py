...
sweep_config = {
    ...
    'parameters': {
        ...
        "batch_samples": {'values': [1024, 2048]}
    }
}
...
def main_wandb():
    ...
    # 1. config 파일 로드
    with open("./portfolio_price_discrete_rebal_step_large_dim/config/train_config.yaml", "r") as f:
        config = yaml.safe_load(f)
    
    with open("./portfolio_price_discrete_rebal_step_large_dim/config/test_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./portfolio_price_discrete_rebal_step_large_dim/model/{traj_len}_v4/'
