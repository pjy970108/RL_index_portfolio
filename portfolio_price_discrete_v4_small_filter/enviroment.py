...
class Stock_Env:
    def __init__(self, config, states, index_df, future_df, eval=False):
        ...
        if eval:
            ...
            date_list = self.index_df.index.unique()
    ...
    def reset(self):
        ...
        if self.eval:
            ...
            # self.idx =  len(self.days) - self.update_interval- self.window_size+1
            self.idx = random.randint(0, len(self.days) - self.update_interval- self.window_size+1)
        # self.idx = 51
        # self.idx = 1429
        # 투자금/weights 초기화
        self.rewards = [1.0]  # 초기 포트폴리오 가치 (수익률 기준)
    ...
    def simulate_combined_portfolio_returns(self, top3_action: dict ) -> pd.Series:
        ...
        for strategy, strat_returns in self.strategy_returns.items():
            ...
            if strategy not in top3_action:
                continue  # strategy가 top3_action에 없으면 그냥 넘어가기
