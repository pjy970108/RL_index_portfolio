...
sweep_config = {
    ...
    "parameters": { 
        ...
        "mu": {"values": [50]}, # Deep Reinforcement Learning Robots for Algorithmic Trading: Considering Stock Market Conditions and U.S. Interest Rates
        "seed" : {'values': [25, 33, 39, 42, 50]}
...
def main_wandb():
    wandb.init(project="sac-sweep", config=sweep_config['parameters'])
    config_wandb = wandb.config
    set_seed(config_wandb.seed)
    
    # 1. config 파일 로드
    with open("./portfolio_price_discrete_SAC_v1/config/train_config.yaml", "r") as f:
        config = yaml.safe_load(f)
    
    with open("./portfolio_price_discrete_SAC_v1/config/test_config.yaml", "r") as f:
    ...
    # 경로 없으면 경로 생성하는 코드
    # 6. 모델 저장 경로 설정
    model_path = f'./portfolio_price_discrete_SAC_v1/model/{traj_len}_seed_v3/'
    ...
    save_filename = (
        ...
        f'mu{config["mu"]}_reward_cond_{config["reward_cond"]}_seed{config_wandb.seed}_sac_model.pth'
    )
    ...
    torch.save(trained_agent.actor.state_dict(), final_path)

    wandb.finish()

if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep_config, project="SAC_portfolio_v2")
